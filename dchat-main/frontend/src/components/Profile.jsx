  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        ...
        const [projectsRes, resourcesRes, seekingRes] = await Promise.all([
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {resources.map((resource) => (
              <span key={resource.id} className="px-3 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                {resource.name}
              </span>
            ))}
            {resources.length === 0 && (
              <span className="text-sm text-gray-500 italic">No resources listed</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mb-6">
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          ...
          <div className="space-y-2">
            ...
            {seeking.length === 0 && (
              <span className="text-sm text-gray-500 italic">No opportunities listed</span>
            )}
          </div>
        </div>
      </div>
