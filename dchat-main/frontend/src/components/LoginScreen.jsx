...
const LoginScreen = ({ onLogin }) => {
  ...
  // Phone Login (with Real Backend Verification)
    ...
    try {
      if (!showVerification) {
        ...
        const response = await fetch('/api/auth/verify-code', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ target: phone, code: verificationCode })
        })
        ...
        // Verification successful, proceed to login/register logic
            ...
            <form onSubmit={handlePhoneLogin} className="space-y-4">
              {!showVerification ? (
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="000000"
                    maxLength={6}
                    className="w-full h-14 px-4 border-2 border-gray-200 rounded-full focus:border-black focus:outline-none transition-colors text-base text-center tracking-widest"
                    disabled={isSubmitting}
                    required
                  />
                </div>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <Phone className="w-5 h-5" />
                    {showVerification ? 'Verify & Login' : 'Continue with Phone'}
                  </>
                )}
              </Button>
            </form>
